...
\begin{figure}
    \centering
    \subcaptionbox{Cliff Walking Environment}{
        \centering
        \includegraphics[width=0.45\textwidth]{figures/cliff-env.pdf}
        \label{fig:cliff-env}
    }
    % \hspace{0.5cm}
    \subcaptionbox{The effect of rollout dropout}{
        \centering
        \includegraphics[width=0.45\textwidth]{figures/cliffwalking.pdf}
        \label{fig:cliff-dis}
    }
...
表\ref{tab:cliff-stats}显示了在CliffWalking环境下使用单层或双层重放缓冲区的PER算法统计结果。这些指标是落入悬崖区的概率，以及平均收益。每次实验的运行由3组随机种子生成，每次实验需要2000步。可以看出，当使用双层回放时，PER算法的表现比单层回放缓冲器的整体表现要好。
...
在本实验中，我们关于样本效率对比了多个基线方法，具体而言，我们将基于模型的强化学习算法中的代表算法MBPO\cite{janner2019trust}，以及它与PER \cite{schaul2016prioritized}的结合MBPO+PER进行了对比实验。

在不同环境的实验中所使用的参数设定如表~\ref{tab:parameters}所示。
